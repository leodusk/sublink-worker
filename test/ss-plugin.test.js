...
describe('SS Plugin Issue #290', () => {
  ...
  it('should force Clash-compatible v2ray-plugin fields for SS websocket nodes', async () => {
    ...
    const yamlText = await builder.build();
    const built = yaml.load(yamlText);
    const proxy = built.proxies.find(p => p.name === '日本7');

    expect(proxy).toBeDefined();
    expect(proxy.plugin).toBe('v2ray-plugin');
    expect(proxy['plugin-opts']).toMatchObject({
      mode: 'websocket',
      host: 'invent.rpt.de5.net',
      path: '/perfect-ss/?ed=2560',
      tls: true,
      peer: 'invent.rpt.de5.net',
      mux: false,
      allowInsecure: true
    });
