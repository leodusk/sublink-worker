...
function normalizeShadowsocksPluginOpts(proxy) {
    if (proxy.plugin !== 'v2ray-plugin' || !proxy.plugin_opts) {
        return proxy.plugin_opts;
    }

    const opts = { ...proxy.plugin_opts };
    const peer = opts.peer || opts.sni || opts.host;
    ...
    opts.tls = parsePluginBool(opts.tls, true);
    opts.mux = false;
    opts.allowInsecure = true;

    if (typeof opts['skip-cert-verify'] !== 'undefined') {
        delete opts['skip-cert-verify'];
    }
        ...
        switch (proxy.type) {
            case 'shadowsocks':
                return {
                    ...
                    ...(proxy.plugin !== 'v2ray-plugin' ? { udp: getClashUdpValue(proxy) } : {}),
